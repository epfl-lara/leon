  ...
  def run(problem: Problem) = {
    ...
    val formulas = problem.funDefs.map({ funDef =>
      funDef -> checker.getRelations(funDef).collect({
        case Relation(_, path, FunctionInvocation(fd, args), _) if problem.funDefs(fd) =>
    ...
    val decreasing = formulas.map({ case (fd, formulas) =>
      val solved = formulas.map({ case (fid, (gt, ge)) =>
        if (definitiveALL(gt, fd.precondition)) Success
        else if (definitiveALL(ge, fd.precondition)) Dep(Set(fid))
        else Failure
      })
