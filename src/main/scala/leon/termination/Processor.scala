...
trait Solvable extends Processor {
  ...
  private val solver: SolverFactory[Solver] = SolverFactory(cond => {
    ...
    new FairZ3Solver(context, newProgram, cond).setTimeout(500L)
  })
  ...
  def maybeSAT(problem: Expr, cond: Option[Expr]): Boolean = {
    withoutPosts {
      SimpleSolverAPI(solver).solveSAT(problem, cond)._1 getOrElse true
    }
  }

  def definitiveALL(problem: Expr, cond: Option[Expr]): Boolean = {
    withoutPosts {
      SimpleSolverAPI(solver).solveSAT(Not(problem), cond)._1.map(!_) getOrElse false
    }
  }

  def definitiveSATwithModel(problem: Expr, cond: Option[Expr]): Option[Map[Identifier, Expr]] = {
    withoutPosts {
      val (sat, model) = SimpleSolverAPI(solver).solveSAT(problem, cond)
