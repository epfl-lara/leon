...
import purescala.CollectorWithPaths
...
trait Strengthener { self : TerminationChecker with RelationComparator with RelationBuilder =>
  ...
  def strengthenPostconditions(funDefs: Set[FunDef])(implicit solver: Processor with Solvable) {
    ...
    val callees : Set[FunDef] = funDefs.map(fd => self.program.transitiveCallees(fd)).flatten
    val sortedCallees : Seq[FunDef] = callees.toSeq.sortWith((fd1, fd2) => self.program.transitivelyCalls(fd2, fd1))
    ...
    for (funDef <- sortedCallees if !strengthenedPost(funDef) && funDef.hasBody && self.terminates(funDef).isGuaranteed) {
      def strengthen(cmp: (Expr, Expr) => Expr): Boolean = {
        ...
        val prec = killForallExpressions(matchToIfThenElse(funDef.precondition.getOrElse(BooleanLiteral(true))))
        ...
        if (!solver.definitiveALL(formula, funDef.precondition)) {
  ...
  def strengthenApplications(funDefs: Set[FunDef])(implicit solver: Processor with Solvable) {
    val transitiveFunDefs = funDefs ++ funDefs.flatMap(fd => self.program.transitiveCallees(fd))
    val sortedFunDefs = transitiveFunDefs.toSeq.sortWith((fd1, fd2) => self.program.transitivelyCalls(fd2, fd1))
