...
object Extractors {
  ...
  object Operator extends TreeExtractor[Expr] {
      /* Unary operators */
    private val table = HashMap[Class[_], Expr => Option[(Seq[Expr], Seq[Expr] => Expr)]] (
      ...
      classOf[Choose] -> { (expr: Expr) =>
        val Choose(ce) = expr
        Some((Seq(ce), (es: Seq[Expr]) => Choose(es.head)))
      },
      ...
      classOf[SubString] -> { (expr: Expr) =>
        ...
        Some((t1::a::b::Nil, es => SubString(es(0), es(1), es(2))))
      },
      classOf[BigSubString] -> { (expr: Expr) =>
        val BigSubString(t1, a, b) = expr
        Some((t1::a::b::Nil, es => BigSubString(es(0), es(1), es(2))))
      },
      ...
      classOf[ArrayUpdated] -> { (expr: Expr) =>
        ...
        Some(( Seq(t1, t2, t3), (as: Seq[Expr]) => ArrayUpdated(as(0), as(1), as(2)) ))
      },
    ...
    def unapply(expr: Expr): Option[(Seq[Expr], (Seq[Expr]) => Expr)] = expr match {
      ...
      case other =>
        val fromTable = table.get(expr.getClass)
        if (fromTable.isDefined) fromTable.get.apply(expr)
        else None
    }
  }
