  ...
  def constraintsForCalls(calls: Set[Expr], predEval: (Expr => Boolean)): Seq[Expr] = {
      ...
      val preds = eqs.filter(_ match {
        case BinaryOperator(Variable(lid), Variable(rid), _) => {
          if (lid == rid) false
            ...
            if (lid.getType == Int32Type || lid.getType == RationalType || lid.getType == IntegerType) true
            else false
          }
        }
    ...
    def predForDisequality(call1: Expr, call2: Expr): Seq[Expr] = {
      ...
      if (makeEfficient && ants.exists(_ match {
        case Equals(l, r) if (l.getType != RationalType && l.getType != BooleanType && l.getType != IntegerType) => true
        case _ => false
      })) {
        Seq()
      } else {
          ...
          if (!unsatOtherEq.isDefined) {
            eq match {
              case Equals(lhs @ Variable(_), rhs @ Variable(_)) if !predEval(Equals(lhs, rhs)) => {
                if (lhs.getType != Int32Type && lhs.getType != RationalType && lhs.getType != IntegerType)
        ...
        if (unsatOtherEq.isDefined) Seq() //need not add any constraint
          ...
          /*if (ants.exists(_ match {
              case Equals(l, r) if (l.getType != Int32Type && l.getType != RationalType && l.getType != BooleanType && l.getType != IntegerType) => true
              case _ => false
            })) {
