...
object LinearConstraintUtil {
  ...
  def exprToTemplate(expr: Expr): LinearTemplate = {
    ...
    // FIXME: Is this correct (the literal type used)?
    val BinaryOperator(lhs, InfiniteIntegerLiteral(x), op) = linearExpr
  ...
  def MakeLinear(atom: Expr): Expr = {
    ...
    def PushMinus(inExpr: Expr): Expr = {
      require(inExpr.getType == Int32Type || inExpr.getType == RationalType || inExpr.getType == IntegerType)
    ...
    def PushTimes(mul: Expr, ine: Expr): Expr = {
      ine match {
        ...
        // case InfiniteIntegerLiteral(v) => InfiniteIntegerLiteral(c * v)
    ...
    def simplifyConsts(ine: Expr): (Option[Expr], BigInt) = {
      require(ine.getType == Int32Type || ine.getType == RationalType || ine.getType == IntegerType)
    ...
    def mkLinearRecur(inExpr: Expr): Expr = {
      inExpr match {
        ...
        case fi: FunctionInvocation => fi
        // DEBUG
        // case Assert(_, _, _) => NoTree(inExpr.getType)
        // DEBUG
