...
object ADTConstraint {
  ...
  def apply(e: Expr): ADTConstraint = e match {
    ...
    case Equals(v @ Variable(_), ci @ CaseClassInstanceOf(_, _)) => {
      new ADTConstraint(e, None, Some(e))
    }
    //equals and disequalities betweeen variables
    case Equals(lhs @ Variable(_), rhs @ Variable(_)) if (lhs.getType != Int32Type && lhs.getType != RationalType && lhs.getType != IntegerType) => {
      new ADTConstraint(e, None, None, Some(e))
    }
    case Not(Equals(lhs @ Variable(_), rhs @ Variable(_))) if (lhs.getType != Int32Type && lhs.getType != RationalType && lhs.getType != IntegerType) => {
      new ADTConstraint(e, None, None, Some(e))
    }
    case _ => {
  ...
  def createConstriant(ie: Expr): Constraint = {
    ie match {
      ...
      case Equals(Variable(_), CaseClassSelector(_, _, _))
        ...
        | Equals(Variable(_), Tuple(_))
        | Equals(Variable(_), CaseClassInstanceOf(_, _)) => {

        ADTConstraint(ie)
      }
      case Equals(lhs, rhs) if (lhs.getType != Int32Type && lhs.getType != RationalType && lhs.getType != IntegerType) => {
        //println("ADT constraint: "+ie)
        ADTConstraint(ie)
      }
      case Not(Equals(lhs, rhs)) if (lhs.getType != Int32Type && lhs.getType != RationalType && lhs.getType != IntegerType) => {
        ADTConstraint(ie)
      }
      case _ => {
