...
//          if(freshvar.id.toString == "ifres2"){
//           println("Creating temporary ifres2 type: "+e.getType+" then type: "+thn.getType+" else type: "+elze.getType)
//           println("THen: "+thn)
...
//          }
        ...
        case Let(binder, value, body) => {
          ...
          /*val freshvar = TempIdFactory.createTemp(binder.name,value.getType,true).toVariable
        ...
        case LetTuple(binders, value, body) => {
          ...
          /*val bindvarMap : Map[Expr,Expr] = binders.map((binder) => {
            val bindvar = TempIdFactory.createTemp(binder.name,value.getType,true).toVariable
...
*/
  ...
  def TransformNot(expr: Expr, retainNEQ: Boolean = false): Expr = { // retainIff : Boolean = false
    def nnf(inExpr: Expr): Expr = {
      // println("[DEBUG] Expr " + inExpr + " of type " + inExpr.getType)
      if (inExpr.getType != BooleanType) inExpr
      else inExpr match {
        //matches integer binary relation
        case Not(e @ BinaryOperator(e1, e2, op)) => {
          if (e1.getType == BooleanType || e1.getType == Int32Type || e1.getType == RationalType || e1.getType == IntegerType) {
            e match {
              // case expr @ Equals(lhs, CaseClassInstanceOf(_, _) | CaseClassSelector(_, _, _) | TupleSelect(_, _)) =>
              //   //all case where rhs could use an ADT tree e.g. instanceOF, tupleSelect, fieldSelect
              //   if (lhs.getType == BooleanType && expr.rhs.getType == BooleanType)
              //     Equals(TransformNot(Not(lhs)), expr.rhs)
              //   else {
              //     if(retainNEQ) Not(Equals(lhs,expr.rhs))
              //     else Or(nnf(LessThan(lhs, expr.rhs)), nnf(GreaterThan(lhs, expr.rhs)))
              //   }
              // case expr @ Equals(CaseClassInstanceOf(_, _) | CaseClassSelector(_, _, _) | TupleSelect(_, _), rhs) =>
              //   //all case where lhs could use an ADT tree e.g. instanceOF, tupleSelect, fieldSelect
              //   if (expr.lhs.getType == BooleanType && rhs.getType == BooleanType)
              //     Equals(expr.lhs, TransformNot(Not(rhs)))
              //   else {
              //     if(retainNEQ) Not(Equals(expr.lhs,rhs))
              //     else Or(nnf(LessThan(expr.lhs, rhs)), nnf(GreaterThan(expr.lhs, rhs)))
              //   }
              case e: Equals => {
                if (e1.getType == BooleanType && e2.getType == BooleanType) {
                  //if (retainNEQ) Not(Equals(e1,e2))
                  //else {
                  //  Or(TransformNot(Not(Implies(e1,e2))), TransformNot(Not(Implies(e2,e2))))
                  //}
        ...
        case e @ Equals(lhs, CaseClassInstanceOf(_, _) | CaseClassSelector(_, _, _) | TupleSelect(_, _) | FunctionInvocation(_, _)) =>
          //all case where rhs could use an ADT tree e.g. instanceOF, tupleSelect, fieldSelect, function invocation
          e
        /*case e @ Equals(CaseClassInstanceOf(_, _) | CaseClassSelector(_, _, _) | TupleSelect(_, _), rhs) =>
          //all case where lhs could use an ADT tree e.g. instanceOF, tupleSelect, fieldSelect
         e*/
  ...
  def IntLiteralToReal(inexpr: Expr): Expr = {
    val transformer = (e: Expr) => e match {
      ...
      case IntLiteral(v) => RationalLiteral(v, 1)
      case _ => e
    }
   ...
   */
  def RationalLiteralToInt(inexpr: Expr): Expr = {
    val transformer = (e: Expr) => e match {
      case RationalLiteral(v, bone) => InfiniteIntegerLiteral(v)
      case RationalLiteral(_, _) => throw new IllegalStateException("cannot convert real literal to integer: " + e)
      case _ => e
    }
    simplePostTransform(transformer)(inexpr)
  }
