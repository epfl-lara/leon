...
class Minimizer(ctx: InferenceContext) {
  ...
  var lowerBoundMap = Map[Variable, RationalLiteral]()
  def updateLowerBound(tvar: Variable, rval: RationalLiteral) = {
  ...
  def tightenTimeBounds(timeTemplate: Expr, inputCtr: Expr, initModel: Map[Identifier, Expr]): Map[Identifier, Expr] = {
    ...
    val solver = SimpleSolverAPI(
      new TimeoutSolverFactory(SolverFactory(() => new ExtendedUFSolver(leonctx, program) with TimeoutSolver),ctx.timeout * 1000))
    ...
    orderedTempVars.foldLeft(inputCtr: Expr)((acc, tvar) => {
      ...
      var upperBound = if (currentModel.contains(tvar.id)) {
        currentModel(tvar.id).asInstanceOf[RationalLiteral]
      } else {
        initModel(tvar.id).asInstanceOf[RationalLiteral]
      }
      //note: the lower bound is an integer by construction
      var lowerBound: Option[RationalLiteral] = if (tvar == orderedTempVars(0) && lowerBoundMap.contains(tvar))
    	  										Some(lowerBoundMap(tvar))
    	  										else None

      //a helper method
      def updateState(nmodel: Map[Identifier, Expr]) = {
        upperBound = nmodel(tvar.id).asInstanceOf[RationalLiteral]
      ...
      if (this.debugMinimization) {
        reporter.info("Minimizing variable: " + tvar + " Initial Bounds: [" + upperBound +","+
      ...
      do {
        ...
        if (continue) {
          ...
          if (lowerBound.isDefined && evaluateRealPredicate(GreaterEquals(lowerBound.get, currval))) {
            ...
            res match {
              case Some(true) => {
                //FIXME: The following code for overflow handling is absolete
                //here we have a new upper bound and also a newmodel
                /*val newval = newModel(tvar.id).asInstanceOf[RationalLiteral]
                if (newval.hasOverflow) {
                  ...
                  val flval = RationalLiteral((n / d).intValue, 1)
      ...
      val currval@RationalLiteral(n,d) = currentModel(tvar.id)
      if (d != 1) {
  ...
  def checkBoundingInteger(tvar: Variable, rl: RationalLiteral, nlctr: Expr, solver: SimpleSolverAPI): Option[Map[Identifier, Expr]] = {
    val nl@RationalLiteral(n, d) = normalize(rl)
    if (d != 1) {
      val flval = floor(nl)
  ...
  def computeCompositionLevel(template: Expr): Map[Variable, Int] = {
    ...
    def updateMax(v: Variable, level: Int) = {
      reporter.info("Nesting level: " + v + "-->" + level)
