      ...
      if(isRecursive) {
        ...
        if (shouldCreateVC(recFun)) {
          ...
          val resvar = if (recFun.hasPostcondition) {
            ...
            val origRes = getResVar(recFun).get
        ...
        reporter.info("Unrolling " + Equals(call.retexpr,call.fi))
        inilineCall(call, formula)
      }
      else {
        //here we are unrolling a non-recursive function
        reporter.info("Inlining "+Equals(call.retexpr,call.fi))
        inilineCall(call, formula)
      }
    } else Set()
  }
