...
import leon.evaluators.OrbEvaluator
...
class SpecInstantiator(ctx: InferenceContext, program: Program, ctrTracker: ConstraintTracker) {
   ...
   */
  def axiomsForCalls(formula: Formula, calls: Set[Call], model: LazyModel, tmplMap: Map[Identifier,Expr], eval: OrbEvaluator): Seq[Constraint] = {
