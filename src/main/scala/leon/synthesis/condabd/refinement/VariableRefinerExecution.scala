...
class VariableRefinerExecution(variableDeclarations: Seq[Declaration],
  ...
  for (varDec <- variableDeclarations) {
    varDec match {
      case Declaration(_, _, typeOfVar: ClassType, ImmediateExpression(_, IsTyped(Variable(id), act @ AbstractClassType(cd, _)))) =>
        variableRefinements += (id -> MutableSet(act.knownDescendents: _*))
      case _ =>
    }
  }
  ...
  override def checkRefinements(expr: Expr, condition: Expr)(evaluator: Evaluator = null) = {
    ...
    if (variables.size == 1) {
      ...
      variable match {
        case oldId @ IsTyped(id, act @ AbstractClassType(cd, _)) // do not try to refine if we already know a single type is possible
        if variableRefinements(id).size > 1 =>

          assert(variableRefinements(id) subsetOf act.knownDescendents.toSet)

          val optCases =
            for (dcd <- variableRefinements(id)) yield dcd match {
