...
case object HOFDecomp extends Rule("HOFDecomp") {
  def instantiateOn(implicit hctx: SearchContext, p: Problem): Traversable[RuleInstantiation] = {
    ...
    def getCandidates(fd: FunDef): Traversable[RuleInstantiation] = {

      fcallFinder.find(fd, tpe).flatMap { tfd =>

        val tpsMap1 = (fd.tparams.map(_.tp) zip tfd.tps).toMap
        ...
        val altsPerArgs = tfd.params.zipWithIndex.map { case (vd, i) =>
          if (isHOFParam(vd)) {
            ...
            val compatibleInputs = {
              p.as.filter(a => instantiation_<:(a.getType, vd.getType).nonEmpty)
            }
        ...
        val calls = cartesianProduct(altsPerArgs).flatMap { vs =>
          ...
          if (vs.exists(v => asSet(v))) {
            Some((FunctionInvocation(tfd, vs.map(_.toVariable)), hofId))
          } else {
            None
          }
        }
        ...
        val results = for ((c, hofId) <- calls) yield {
          hctx.reporter.debug("Considering call: "+c.asString)
          ...
          try {
            ...
            solver.check match {
              case Some(true) =>
                ...
                hctx.reporter.debug(eb.asString("Tests:"))
