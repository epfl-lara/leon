...
class SerialInstrumenter(ctx: LeonContext, program: Program) {
  ...
  lazy val funcInsts = {
    ...
    instToInstrumenter.values.foreach{ m =>
      // TODO: this should become more general to accommodate 'timeBody'
      m.functionsToInstrument.foreach(fd => update(fd, m.inst))
    }
    emap.toMap
  }
  lazy val instFuncs = funcInsts.keySet //should we exclue theory operations ?
  ...
  /**
   * Index of the inssrumentation 'inst' in result tuple that would be created.
          ...
          case f @ FunctionInvocation(tfd, args) =>
            ...
            if (instFuncs(tfd.fd)) {
              ...
              val instexprs = instrumenters.map { m =>
                ...
                m.instrument(e, subeInsts.getOrElse(m.inst, List()) ++ calleeInst)
              }
...
abstract class Instrumenter(program: Program, si: SerialInstrumenter) {
   ...
   */
  def instrument(e: Expr, subInsts: Seq[Expr])(implicit fd: FunDef, letIdMap: Map[Identifier, Identifier]): Expr
