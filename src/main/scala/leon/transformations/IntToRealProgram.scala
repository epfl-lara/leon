...
abstract class ProgramTypeTransformer {
  ...
  def mapType(tpe: TypeTree): TypeTree = {
    tpe match {
      case t @ RationalType => mapNumericType(t)
  ...
  def transform(program: Program): Program = {
    ...
    newFundefs = program.definedFunctions.map((fd) => {
      ...
      val newfd = new FunDef(FreshIdentifier(fd.id.name, newFunType, true), fd.tparams, mapType(fd.returnType), fd.params.map(mapDecl), DefType.MethodDef  )
      (fd, newfd)
    }).toMap
...
class RealToIntProgram extends ProgramTypeTransformer {
  ...
  def mapLiteral(lit: Literal[_]) : Literal[_] = lit match {
    ...
    case RationalLiteral(_,_) => throw new IllegalStateException("Cannot convert real to integer: "+lit)
    case _ => lit
  }
