...
class NonlinearityEliminator(skipAxioms: Boolean, domain: TypeTree) {
  ...
  val pivMultFun = {
    ...
    mfd.addAnnotation("theoryop", "monotonic") //"distributive"
    mfd
  }
  ...
  val multFun = {
    ...
    val args = Seq(xid, yid)
    val funcType = FunctionType(Seq(domain, domain), domain)
    val fd = new FunDef(FreshIdentifier("mult", funcType, false), Seq(), domain, args.map((arg) => ValDef(arg, Some(arg.getType))), DefType.MethodDef)
    ...
    fd.addAnnotation("theoryop", "monotonic") //"distributive"
    fd
  }
  ...
  def apply(program: Program): Program = {
    ...
    newFundefs.foreach((entry) => {
      ...
      newfd.postcondition = if (fd.postcondition.isDefined) {
        ...
        val newpost = if (tmplExpr.isDefined) {
