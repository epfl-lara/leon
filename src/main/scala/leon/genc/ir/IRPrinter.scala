...
final class IRPrinter[S <: IR](val ir: S) {
  ...
  private def rec(alloc: ArrayAlloc)(implicit ptx: Context): String = {
    (alloc: @unchecked) match {
      case ArrayAllocStatic(arrayType, length, values) => "Array[" + rec(arrayType.base) + "](" + (values map rec mkString ", ") + ")"
      case ArrayAllocVLA(arrayType, length, valueInit) => "Array[" + rec(arrayType.base) + "].fill(" + rec(length) + ")(" + rec(valueInit) + ")"
    }
  }
