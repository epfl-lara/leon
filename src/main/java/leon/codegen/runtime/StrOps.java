...
public class StrOps {
	...
	public static String substring (String a, BigInt start, BigInt end) {
      if(start.greaterEquals(end) || start.greaterEquals(length(a)) || end.lessEquals(new BigInt("0")))
    	  return "";
      else
    	  return a.substring(start.underlying().intValue(), end.underlying().intValue());
    }
