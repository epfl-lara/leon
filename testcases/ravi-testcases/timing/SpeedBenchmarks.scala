...
object SpeedBenchmarks {
  ...
  } ensuring(res => (size(res._2._1) <= k && size(res._2._2) <= k) template((a,b) => time <= a*size(str1) + b)) 
  ...
  } ensuring(res => (sizeBound(res._2,k) && (res._1 == Nil() || (size(res._1) <= k && length(res._2) <= length(s) - 1))) template((a,b) => time <= a*length(s) + b))
    ...
    if (res == false) false
      ...
      val (fstr2, sb2) =
        if (nstr2 == Nil())
          ...
          (nstr2, s2)
      fstr1 match {
        case Nil() => if(fstr2 == Nil()) true else false
        case _ => {
          if (fstr2 == Nil()) false
          else {
  ...
  } ensuring (res => true template ((a, b, c, d, e) => time <= a * mult(k, length(s1) + length(s2)) + b * k + c * length(s1) + d * length(s2) + e))
