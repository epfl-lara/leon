    ...
    (str1, str2) match {
      ...
      case _ =>{
        ...
        (s1,s2) match {
          ...
          case (Empty(),Chunk(nstr2, next2)) => Equals(Nil(), nstr2, Empty(), next2, k)
          case (Chunk(nstr1, next1), Empty()) => Equals(nstr1, Nil(), next1, Empty(), k)
          case _ => true          
        }
      }
    }    
  } ensuring(res => true template((a,b,c,d, e) => time <= a*mult(k, length(s1) + length(s2)) + e*k + b*length(s1) + c*length(s2) + d))
