...
object ConcatVariations {
  ...
  def genL(n: Int): List = {
    ...
    if (n == 0) Nil()
      ...
      Cons(n, genL(n - 1))
  } ensuring (res => size(res) == n template((a,b) => depth <= a*n + b))
  ...
  } ensuring(res => size(res) == n*m template((a,b,c,d) => depth <= a*(n*m) + b*n + c*m +d))
  ...
  } ensuring(res => size(res) == n*m template((a,c,d,e,f) => depth <= a*((n*m)*m)+c*(n*m)+d*n+e*m+f)) 
}
