...
object ListOperations {
  ...
  } ensuring (res => size(res) == size(l) template((a,b) => depth <= a*(size(l)*size(l)) + b))
  ...
  }) ensuring (res => true template((a,b) => depth <= a*size(list) + b))
  ...
  def distinct(l: List): List = (
   ...
   }) ensuring (res => size(l) >= size(res) template((a,b) => depth <= a*(size(l)*size(l)) + b))
}
