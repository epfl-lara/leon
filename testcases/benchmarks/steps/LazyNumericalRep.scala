...
object LazyNumericalRep {
  ...
  def main(args: Array[String]) {
    ...
    for (i <- 0 until ops) {
      lazynum = timed { incAndPay(lazynum) } { totalTime += _ }
    }
    println(s"Ephemeral Amortized Time Lazy: ${totalTime / 1000.0}s") // this should be linear in length for both cases
