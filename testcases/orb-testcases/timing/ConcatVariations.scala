...
object ConcatVariations {
  ...
  def genL(n: BigInt): List = {
    ...
    if (n == 0) Nil()
      ...
      Cons(n, genL(n - 1))
  } ensuring (res => size(res) == n && tmpl((a,b) => steps <= a*n + b))
  ...
  } ensuring(res => size(res) == n*m && tmpl((a,b,c,d) => steps <= a*(n*m) + b*n + c*m +d))
  ...
  } ensuring(res => size(res) == n*m && tmpl((a,c,d,e,f) => steps <= a*((n*m)*m)+c*(n*m)+d*n+e*m+f))
}
