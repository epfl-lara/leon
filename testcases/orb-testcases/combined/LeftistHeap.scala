...
object LeftistHeap {
  ...
  } ensuring(res => true && tmpl((a,b,c,d) => time <= a*leftRightHeight(h) + b && rec <= c*leftRightHeight(h) + d))
  ...
  } ensuring(res => true && tmpl((a,b,c,d,e) => time <= a*rightHeight(h1) + b*rightHeight(h2) + c && rec <= d*rightHeight(h1) + e*rightHeight(h2) + c))
  ...
  } ensuring(res => true && tmpl((a,b,c,d) => time <= a*rightHeight(heap) + b && rec <= c*rightHeight(heap) + d))
}
