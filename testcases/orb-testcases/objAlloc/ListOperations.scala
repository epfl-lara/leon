...
object ListOperations {
  ...
  }) ensuring (res =>  size(l1) + size(l2) == size(res) && tmpl((a,b) => alloc <= a*size(l1) + b))
  ...
  } ensuring (res => size(l) == size(res) && tmpl((a,b) => alloc <= a*size(l) + b))
  ...
  } ensuring (res => size(res) == size(l) && tmpl((a,b) => alloc <= a*(size(l)*size(l)) + b))
  ...
  }) ensuring (res => true && tmpl((a,b) => alloc <= a*size(list) + b))
  ...
  def distinct(l: List): List = (
   ...
   }) ensuring (res => size(l) >= size(res) && tmpl((a,b) => alloc <= a*(size(l)*size(l)) + b))
}
