...
object Knapscak {
  ...
  case class Cons(x: (BigInt, BigInt), tail: IList) extends IList // a list of pairs: (weight, value)
  case class Nil() extends IList

  /**
   * A property that holds if the `solveForWeight` is cached for all
   * weights lesser than or equal to `i`.
   */
  ...
  /**
   * Lemma: Monotonicity of deps with respect to the state (keys of the cache)
   */
  @invisibleBody
  @traceInduct
  ...
  /**
   * Lemma: forall. x, x <= y && deps(y) => deps(x)
   */
  @traceInduct
  ...
  /**
   * Computes the optimal value of filling a knapsack of weight `w` using `remItems`
   */
  @invstate
  def maxOverItems(items: IList, w: BigInt, remItems: IList): BigInt = {
    ...
    remItems match {
      case Cons((wi, vi), tail) =>
        ...
        if (wi <= w && wi > 0) {
          val maxWithItem = vi + solveForWeight(w - wi, items)
  ...
  } ensuring (_ => steps <= ? * remItems.size + ?)

  /**
   * Computes the optimal value of filling a knapsack of weight `w` using `items`
   */
  @memoize
  ...
  /**
   * A helper function that specifies properties ensured by an invocation of
   * `solveForWeight`.
   */
  @invisibleBody
  def solveForWeightHelper(i: BigInt, items: IList) = {
    ...
    (i == 0 || depsMono(i - 1, items, inSt[BigInt], outSt[BigInt])) &&
      deps(i, items) &&
      steps <= ? * items.size + ?
  })
  ...
  def main(args: Array[String]) {
    ...
    val reslist1 = knapsack(100, weightsnValues1) // without memoization this will exponentially solver
