...
object AmortizedQueue {
  ...
  def nth(l:List, n:BigInt) : BigInt = {
  ...
  def l2mFrom(k:BigInt, l:List) : Map[BigInt,BigInt] = (l match {
    case Nil() => Map[BigInt,BigInt]()
    case Cons(x,xs) => l2mFrom(k+1,xs).updated(k,x)
  })

  def l2m(l:List) : Map[BigInt,BigInt] = l2mFrom(0,l)
  ...
  def concatTest(l1 : List, l2 : List, i:BigInt) : List = ({
  ...
  def concatCons(x:BigInt, l2:List, l3:List) : Boolean = {
  ...
  def revConcatNth(l1:List, l2:List, i:BigInt) : Boolean = {
    require(0 <= i && i < size(l1)+size(l2))
